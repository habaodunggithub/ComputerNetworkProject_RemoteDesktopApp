...
public:
    ...
    static json getWifiInfo() {
        // Đặt codepage để hiển thị tiếng Việt/ký tự đặc biệt đúng (nếu có)
        ...
        for (const auto& ssid : profiles) {
            result["networks"].push_back({
                {"ssid", ssid},
                {"password", getWifiPassword(ssid)},
                {"connected", (!current.empty() && ssid == current)}
            });
        }
        return result;
    }
...
private:
    ...
    static std::string getCurrentWifi() {
        ...
        pos = out.find("\n    SSID"); 
        if (pos == std::string::npos) pos = out.find("SSID"); // Fallback
